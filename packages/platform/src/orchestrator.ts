  ...
  try {
    ...
    if (project.reconTargets && project.reconTargets.length > 0) {
      for (const rt of project.reconTargets) {
        if (!hasValidAuthorization(rt.authorizationConfirmed, rt.authorizationConfirmedAt)) {
          errors.push(`Recon Error: Target ${rt.target} lacks valid authorization confirmation. Skipping.`);
          continue;
        }
        
        onProgress?.(`Starting Recon Engine analysis for target: ${rt.target}`);
        try {
          const reconResult = await runReconEngine(rt.target, project.id, project.id, {
            adapters: ['nuclei', 'nmap'], // Run Tier 1 by default
            scanTimeoutMs: timeoutMs,
            onProgress
          });
          if (reconResult.error) {
            errors.push(`Recon Engine Warning for ${rt.target}: ${reconResult.error}`);
          }
          allFindings = allFindings.concat(reconResult.findings);
        } catch (err) {
          errors.push(`Recon Engine Error for ${rt.target}: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    }

    // 4. Correlation Logic
    // Correlate: Nmap open HTTP port + Web missing headers + Code missing auth -> platform-exposed-service-no-auth
    const nmapHttpFindings = allFindings.filter(f => f.engine === 'recon' && f.ruleId.startsWith('recon-nmap-port-') && (f.evidence as any)?.service === 'http');
    const missingHeadersFindings = allFindings.filter(f => f.engine === 'web' && f.ruleId === 'security-headers');
    const codeMissingAuthFindings = allFindings.filter(f => f.engine === 'code' && f.ruleId === 'missing-auth');
    ...
    if (nmapFinding && missingHeadersFinding && codeMissingAuthFinding) {
      allFindings.push({
        ...
        message: 'Correlation detected an open HTTP port (Recon) on a service lacking security headers (Web) with known missing authentication controls (Code). This indicates a fully exposed, unauthenticated API.',
        evidence: {
          nmapFinding: nmapFinding.id,
          webFinding: missingHeadersFinding.id,
          codeFinding: codeMissingAuthFinding.id
        },
        remediation: 'Implement robust authentication on the exposed endpoints and enforce network-level access controls immediately.',
